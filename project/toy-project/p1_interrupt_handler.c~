...
__interrupt_vec(PORT2_VECTOR) Port_1(){
  ...
  else if (P2IFG & SW2) {	      /* did a button cause this interrupt? */
    P2IFG &= ~SW2;		      /* clear pending sw interrupts */
    switch_interrupt_handler();	/* single handler for all switches */
  }
  else if (P2IFG & SW3) {	      /* did a button cause this interrupt? */
    P2IFG &= ~SW3;		      /* clear pending sw interrupts */
    switch_interrupt_handler();	/* single handler for all switches */
  }
  else if (P2IFG & SW4) {	      /* did a button cause this interrupt? */
